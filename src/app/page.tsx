export const dynamic = "force-dynamic"

import { getServerSession } from "next-auth"
import { authOptions } from "@/lib/auth"
import { getNowPlaying } from "@/lib/spotify"
import NowPlaying from "@/components/ui/NowPlaying"
import Link from "next/link"

export default async function Home() {
  const session = await getServerSession(authOptions)

  if (!session?.accessToken) {
    return (
      <main className="min-h-screen bg-black text-white flex flex-col items-center justify-center px-4">
        <div className="text-center max-w-md">
          <h1 className="text-5xl font-bold mb-4 tracking-tight">Rewindify</h1>
          <p className="text-white/50 text-lg mb-10">Your music has a memory.</p>
          <a
            href="/api/auth/signin/spotify"
            className="inline-flex items-center gap-3 bg-[#1DB954] hover:bg-[#1ed760] text-black font-semibold px-8 py-3 rounded-full transition-colors"
          >
            <svg viewBox="0 0 24 24" className="w-5 h-5 fill-current" xmlns="http://www.w3.org/2000/svg">
              <path d="M12 0C5.4 0 0 5.4 0 12s5.4 12 12 12 12-5.4 12-12S18.66 0 12 0zm5.521 17.34c-.24.359-.66.48-1.021.24-2.82-1.74-6.36-2.101-10.561-1.141-.418.122-.779-.179-.899-.539-.12-.421.18-.78.54-.9 4.56-1.021 8.52-.6 11.64 1.32.42.18.479.659.301 1.02zm1.44-3.3c-.301.42-.841.6-1.262.3-3.239-1.98-8.159-2.58-11.939-1.38-.479.12-1.02-.12-1.14-.6-.12-.48.12-1.021.6-1.141C9.6 9.9 15 10.561 18.72 12.84c.361.181.54.78.241 1.2zm.12-3.36C15.24 8.4 8.82 8.16 5.16 9.301c-.6.179-1.2-.181-1.38-.721-.18-.601.18-1.2.72-1.381 4.26-1.26 11.28-1.02 15.721 1.621.539.3.719 1.02.419 1.56-.299.421-1.02.599-1.559.3z"/>
            </svg>
            Connect Spotify
          </a>
        </div>
      </main>
    )
  }

  const nowPlaying = await getNowPlaying(session.accessToken).catch(() => null)

  return (
    <main className="min-h-screen bg-black text-white px-4 py-10 max-w-2xl mx-auto">
      <div className="mb-10">
        <h1 className="text-3xl font-bold mb-1">Rewindify</h1>
        <p className="text-white/50 text-sm">Your music has a memory.</p>
      </div>

      <NowPlaying track={nowPlaying} />

      <div className="grid grid-cols-2 gap-4">
        <Link
          href="/ghost-songs"
          className="rounded-2xl bg-white/5 hover:bg-white/10 transition-colors p-6 group"
        >
          <p className="text-lg font-bold text-violet-400 mb-1 group-hover:opacity-80 transition-opacity">Ghost Songs</p>
          <p className="text-sm text-white/40">Songs you loved and forgot.</p>
        </Link>
        <Link
          href="/past-station"
          className="rounded-2xl bg-white/5 hover:bg-white/10 transition-colors p-6 group"
        >
          <p className="text-lg font-bold text-amber-400 mb-1 group-hover:opacity-80 transition-opacity">Past Station</p>
          <p className="text-sm text-white/40">How your taste is shifting.</p>
        </Link>
      </div>
    </main>
  )
}
